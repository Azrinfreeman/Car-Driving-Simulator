...
using Random = UnityEngine.Random;
    ...
    public void PlayHandBrake()
    {
        ...
        if (!handBrake.isPlaying && myCar.velocity.magnitude < 3)
        {
            ...
            handBrake.Play();
            if (myCar.velocity.magnitude >1 && myCar.velocity.magnitude <= 2.3f)
            {
                handBrake.Stop();
            }
        }
    ...
    private void FixedUpdate()
        {
            ...
            if (m_StartedSound)
            {
              ...
              //  float pitch = ULerp(lowPitchMin, lowPitchMax, m_CarController.engineRPM / m_CarController.maxRPM);
                float pitch = ULerp(lowPitchMin, lowPitchMax, myCar.velocity.magnitude / 100);
                ...
                // get values for fading the sounds based on the acceleration
                  ...
                  //  accFade = Mathf.Abs((InputManager.vertical > 0 && !m_CarController.test ) ?InputManager.vertical : 0);
                    accFade = Mathf.Abs((Input.GetAxisRaw("Vertical") > 0 ) ? Input.GetAxisRaw("Vertical") : 0);

                    float decFade = 1 - accFade;

                    // get the high fade value based on the cars revs
                    float highFade = Mathf.InverseLerp(0.2f, 0.8f,  myCar.velocity.magnitude / 10000);
